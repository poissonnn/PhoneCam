...
def click(x: int = None, y: int = None, button: str = "left"):
    ...
    if x is not None and y is not None:
        move_to(x, y)
    subprocess.run(["ydotool", "click", _button_code(button)], check=True)
...
def mouse_down(button: str = "left"):
    ...
    subprocess.run(
        ["ydotool", "click", "--button", _button_code(button), "--", "press"],
        check=True,
    )
...
def mouse_up(button: str = "left"):
    ...
    subprocess.run(
        ["ydotool", "click", "--button", _button_code(button), "--", "release"],
        check=True,
    )
